"use client";

import { useMapStore } from "@/stores/mapStore";
import { PROJECTIONS } from "@/lib/projections";
import { X, Info, ExternalLink } from "lucide-react";

export default function MapTruthModal() {
  const { mapTruthModalOpen, setMapTruthModalOpen, activeProjection, selectedCountries } =
    useMapStore();

  if (!mapTruthModalOpen) return null;

  const currentDef = PROJECTIONS[activeProjection] || PROJECTIONS["equal-earth"];
  const selected = selectedCountries[0];

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-in fade-in duration-150 font-sans">
      <div className="relative w-full max-w-xl rounded-2xl bg-slate-900 border border-slate-700 shadow-2xl p-6 text-slate-200">
        <button
          onClick={() => setMapTruthModalOpen(false)}
          className="absolute top-4 right-4 w-7 h-7 rounded-lg bg-slate-800 hover:bg-slate-700 flex items-center justify-center text-slate-400 hover:text-white transition-colors"
        >
          <X className="w-4 h-4" />
        </button>

        <div className="flex items-center gap-2 text-sky-400 font-mono text-xs uppercase tracking-wider mb-1.5">
          <Info className="w-4 h-4" />
          <span>Cartographic Transparency Protocol</span>
        </div>
        <h2 className="text-xl font-bold text-white tracking-tight">Map Truth</h2>
        <p className="text-xs text-slate-400 mt-0.5">
          Exact parameters and data provenance behind the currently rendered canvas.
        </p>

        <div className="mt-5 space-y-2.5 font-mono text-xs">
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 flex justify-between items-center">
            <span className="text-slate-400 font-sans">Active Projection</span>
            <span className="text-white font-semibold">{currentDef.name}</span>
          </div>

          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 flex justify-between items-center">
            <span className="text-slate-400 font-sans">Classification</span>
            <span className="text-emerald-400">{currentDef.type}</span>
          </div>

          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 flex justify-between items-center">
            <span className="text-slate-400 font-sans">Area Property</span>
            <span
              className={
                activeProjection === "equal-earth" || activeProjection === "mollweide"
                  ? "text-emerald-400"
                  : "text-amber-400"
              }
            >
              {activeProjection === "equal-earth" ? "Preserved (1:1 Worldwide)" : "Distorted"}
            </span>
          </div>

          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 flex justify-between items-center">
            <span className="text-slate-400 font-sans">Vector Dataset</span>
            <span className="text-slate-200">Natural Earth 1:110m Admin 0</span>
          </div>

          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 flex justify-between items-center">
            <span className="text-slate-400 font-sans">Area Engine</span>
            <span className="text-sky-400">Turf.js WGS84 Geodesic Integral</span>
          </div>

          {selected && (
            <div className="p-3 rounded-xl bg-sky-950/30 border border-sky-500/30 flex justify-between items-center">
              <span className="text-sky-300 font-sans">Selected Country</span>
              <span className="text-white font-semibold">
                {selected.name} ({selected.iso3})
              </span>
            </div>
          )}
        </div>

        <div className="mt-6 flex justify-between items-center pt-3 border-t border-slate-800 text-xs">
          <a
            href="/methodology"
            className="text-sky-400 hover:text-sky-300 underline flex items-center gap-1 font-mono"
          >
            <span>Full Calculation Methodology</span>
            <ExternalLink className="w-3 h-3 inline" />
          </a>
          <button
            onClick={() => setMapTruthModalOpen(false)}
            className="px-4 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium"
          >
            Dismiss
          </button>
        </div>
      </div>
    </div>
  );
}
